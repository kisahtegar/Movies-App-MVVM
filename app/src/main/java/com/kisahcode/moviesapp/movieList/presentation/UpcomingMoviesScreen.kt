...
/**
 * A composable function to display a grid of upcoming movies.
 *
 * This function checks the state of the upcoming movie list. If the list is empty, it displays a
 ...
 *
 * @param movieListState The current state of the movie list which includes the list of upcoming movies and loading status.
...
fun UpcomingMoviesScreen(
    ...
    if (movieListState.upcomingMovieList.isEmpty()) {
        ...
        // Display the upcoming movies in a grid layout if the list is not empty
        LazyVerticalGrid(
            ...
            // Iterate through the list of upcoming movies
            items(movieListState.upcomingMovieList.size) { index ->
                // Display each movie item using the MovieItem composable
                MovieItem(
